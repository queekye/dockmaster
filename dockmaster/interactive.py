...
def configure_project(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # 基本配置
    print("\n--- 基本项目配置 ---")
    print("注意：此命令只配置基本项目信息。其他高级配置（如定时任务、备份等）请使用专门的命令或直接编辑配置文件。\n")
    
    updated_config['project']['name'] = questionary.text(
        "项目名称",
        default=config['project'].get('name', '')
    ).ask()

    # 镜像配置
    updated_config['image']['name'] = questionary.text(
        "镜像名称",
        default=config['image'].get('name', '')
    ).ask()
    ...
    updated_config['container']['name'] = questionary.text(
        "容器名称",
        default=config['container'].get('name', '')
    ).ask()
...
def configure_cleanup(image_summary: Dict[str, Any]) -> Dict[str, Any]:
    ...
    print(f"当前项目共有 {project_images_count} 个镜像，占用空间约 {round(project_images_size, 2)} MB")
    ...
    if image_summary.get('actual_disk_usage'):
        ...
        if image_summary['total_size'] > 0:
            sharing_ratio = (image_summary['total_size'] - image_summary['actual_disk_usage']) / image_summary['total_size'] * 100
    ...
    if image_summary['project_images']:
        ...
        for i, img in enumerate(image_summary['project_images']):
            print(f"  {i+1}. {img['full_tag']} - 创建于 {img['created_ago']} 天前，大小 {img['size_mb']} MB")
    ...
    # 是否保留latest标签
    keep_latest = questionary.confirm(
        "是否保留latest标签的镜像?",
        default=True
    ).ask()
...
def configure_schedule() -> Dict[str, Any]:
    ...
    if task_type == "backup":
        config["cleanup"] = questionary.confirm(
            "是否在备份前清理容器?",
            default=False
        ).ask()

        config["auto_push"] = questionary.confirm(
            "是否自动推送备份镜像?",
            default=False
        ).ask()
