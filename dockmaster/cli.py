...
from dockmaster.cli_utils import (
    ProjectContext,
    check_config_exists,
...
# 创建CLI应用
app = typer.Typer(
    help="Docker项目管理工具", no_args_is_help=True, add_completion=False, rich_markup_mode="rich"
)
        ...
        config_updates = {
            ...
            "image": {
                ...
                "registry": {
                    ...
                    "prefix": updated_fields["image"]["registry"]["prefix"],
                },
            },
...
def build_image(
    ...
    build_args: list[str] = typer.Option(
        [], "--build-arg", help="构建参数，格式：KEY=VALUE", callback=lambda x: x or []
    ),
...
def push_image(
    ...
    try:
        ...
        # 如果指定了prefix，更新配置
        if prefix:
            config_updates = {"image": {"registry": {"prefix": prefix}}}
        ...
        if not password:
            ...
            if not password:
                password = (
                    project_manager.config.get("image", {}).get("registry", {}).get("password")
                )
...
def schedule_task(
    task_type: str = typer.Argument(
        None, help="任务类型：backup/cleanup/list/remove/start/stop/restart/logs"
    ),
    ...
    run_now: bool = typer.Option(False, "--run-now", help="设置任务后立即执行一次"),
):
    """配置定时任务"""
    from .commands.schedule import handle_schedule_command

    handle_schedule_command(task_type, cron, force, lines, follow, run_now)
...
def cleanup_images(
    ...
    keep_latest: bool = typer.Option(
        True, "-l", "--keep-latest/--no-keep-latest", help="是否保留latest标签的镜像"
    ),
    ...
    try:
        ...
        to_delete, to_keep = project_manager.image_manager.cleanup_images(
            keep_latest=keep_latest, keep_days=days, keep_count=count, dry_run=dry_run
        )
