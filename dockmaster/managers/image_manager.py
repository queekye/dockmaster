...
from ..utils import create_temp_file, run_command
...
class ImageManager(BaseManager):
    ...
    def __init__(self, project_dir: str, image_name: str) -> None:
        """
        初始化镜像管理器
    ...
    def push_image(
        ...
        try:
            ...
            if username and password:
                ...
                if username:
                    logger.warning(
                        f"如果失败，请尝试先使用 'docker login -u {username}' 命令手动登录"
                    )
                else:
            ...
            # 推送镜像
            push_output = []
            for line in self.docker_client.images.push(target_image, stream=True, decode=True):
                if "error" in line:
        ...
        try:
            ...
            for repo, images in repo_images.items():
                ...
                # 按数量筛选
                    ...
                    # 过滤出时间戳格式的标签
                    timestamp_images = [
                        img for img in images if re.match(r"^\d{8}_\d{6}$", img["tag"])
                    ]
