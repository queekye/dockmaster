...
def handle_schedule_command(
    ...
    try:
        ...
        if not task_type or not cron:
            _handle_interactive_schedule(project_manager, force)
        else:
            _handle_cli_schedule(project_manager, task_type, cron)
...
def _handle_interactive_schedule(project_manager, force: bool) -> None:
    """处理交互式调度配置"""
    ...
    success = False
    if task_type == "backup":
        success = project_manager.container_manager.schedule_backup(
            schedule_config, None, config["cleanup"], config["auto_push"]
        )
        if success:
            logger.success(f"已设置备份任务: {schedule_config}")
    elif task_type == "cleanup":
        success = project_manager.container_manager.schedule_cleanup(schedule_config, config["paths"])
        if success:
    ...
    # 如果用户选择立即启动调度器
    if success and config.get("start_scheduler", True):
...
def _handle_cli_schedule(project_manager, task_type: str, cron: str) -> None:
    """处理命令行方式的调度配置"""
    # 创建一个简单的每日定时配置
    schedule_config = {
        "type": "daily",
    ...
    success = False
    if task_type == "backup":
        success = project_manager.container_manager.schedule_backup(schedule_config)
        if success:
            logger.success(f"已设置备份任务: 每天 {cron}")
    elif task_type == "cleanup":
        success = project_manager.container_manager.schedule_cleanup(schedule_config)
        if success:
    ...
    # 命令行方式默认启动调度器
    if success and not project_manager.container_manager.is_scheduler_running():
        if project_manager.container_manager.start_scheduler():
            logger.success("调度器已启动")
        else:
            logger.error("启动调度器失败")
